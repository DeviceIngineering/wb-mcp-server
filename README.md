...
> **Перед первым запуском.** В текущем `main` сборка «с нуля» подтягивает библиотеку
> `mcp` 2.x, с которой сервер не стартует. Починка лежит в ветке `fix/startup` (PR #2);
> до её слияния используйте обходной путь из раздела
> [«Как это устроено»](#как-это-устроено), пункт про версию `mcp`.
...
- В текущем `main` **`POST /messages` тоже не проверяет токен**: авторизация стоит только
  на `GET /sse`, то есть защита неполная. Исправление есть в ветке `fix/startup` (PR #2) —
  после его слияния проверка работает на обоих эндпоинтах. До тех пор держите порт 8001
  в доверенной сети и не считайте `MCP_AUTH_TOKEN` достаточной защитой.
...
- **В логах контейнера на каждое MCP-сообщение появляется** `RuntimeError: Unexpected ASGI
  ...
  клиенты, которые переиспользуют keep-alive, могут споткнуться. Это тоже исправлено
  в ветке `fix/startup` (PR #2).
- **Версия библиотеки `mcp` — известная проблема.** Сервер написан под декораторное API
  ...
  Ограничение `<2.0.0` добавлено в ветке `fix/startup` (PR #2). До её слияния обходной
  путь — поставить библиотеку 1.x явно:
